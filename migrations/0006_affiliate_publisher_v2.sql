...
CREATE TABLE clip_products (
 ...
 sort_order INTEGER NOT NULL DEFAULT 100, PRIMARY KEY(clip_id,product_id)
);
CREATE INDEX idx_clip_products_product ON clip_products(product_id);
CREATE TABLE collections (
 id INTEGER PRIMARY KEY AUTOINCREMENT, slug TEXT NOT NULL UNIQUE, title TEXT NOT NULL, description TEXT,
 status TEXT NOT NULL DEFAULT 'draft' CHECK(status IN ('draft','published','archived')),
...
CREATE TABLE collection_items (
 ...
 sort_order INTEGER NOT NULL DEFAULT 100, PRIMARY KEY(collection_id,product_id)
);
...
CREATE TABLE page_views (
 ...
 source TEXT, campaign TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
